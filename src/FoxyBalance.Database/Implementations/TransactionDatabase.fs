...
type TransactionDatabase(options : IDatabaseOptions) =
    ...
    let mapRowToTransaction (read : RowReader) : Transaction =
        { Id = read.int64 "id"
          ...
          RecurringBillId = read.int64OrNone "recurringbillid"
          AutoGenerated = read.boolOrNone "autogenerated" |> Option.defaultValue false
          MatchedTransactionId = read.int64OrNone "matchedtransactionid" }
    ...
    interface ITransactionDatabase with
                ...
                if List.isEmpty partialTransactions then
                    ...
                    if List.isEmpty transactionsToImport then
                        ...
                        for transaction in transactionsToImport do
                            ...
                            // recurringbillid
                            match transaction.RecurringBillId with
                            | Some billId ->
                                do! writer.WriteAsync(billId, NpgsqlTypes.NpgsqlDbType.Bigint)
                            | None ->
                                do! writer.WriteAsync(DBNull.Value, NpgsqlTypes.NpgsqlDbType.Bigint)

                            // autogenerated
                            do! writer.WriteAsync(transaction.AutoGenerated, NpgsqlTypes.NpgsqlDbType.Boolean)
        ...
        member _.CreateAsync(userId, transaction) =
            ...
            let data = [
                ...
                "autoGenerated", Sql.bool transaction.AutoGenerated
                "matchedTransactionId", Sql.dbnull
            ]
            ...
            |> Sql.query """
                ...
                RETURNING id, datecreated, matchedtransactionid
            """
            |> Sql.parameters data
        ...
        member _.UpdateAsync(userId, transactionId, transaction) =
            ...
            |> Sql.query """
                ...
                WHERE userid = @userId AND id = @id
                RETURNING datecreated, importid, recurringbillid, autogenerated, matchedtransactionid
                """
            |> Sql.parameters data
            |> Sql.executeRowAsync (fun read ->
                {
                    ...
                    ImportId = read.stringOrNone "importid"
                    RecurringBillId = read.int64OrNone "recurringbillid"
                    AutoGenerated = read.bool "autogenerated"
                    MatchedTransactionId = read.int64OrNone "matchedtransactionid"
                })
        ...
        member _.ListMatchCandidatesAsync(userId, cutoffDate) =
            connection
            |> Sql.query """
                SELECT * FROM foxybalance_transactions
                WHERE userid = @userId
                  AND recurringbillid IS NULL
            ...
            |> Sql.parameters [
                "userId", Sql.int userId
                "cutoffDate", Sql.timestamptz (cutoffDate.ToUniversalTime())
            ]
            |> Sql.executeAsync mapRowToTransaction
            |> Sql.map Seq.ofList

        member _.GetAutoGeneratedForBillAsync(userId, billId) =
            connection
            |> Sql.query """
                SELECT * FROM foxybalance_transactions
                WHERE userid = @userId
                  AND recurringbillid = @billId
            ...
            |> Sql.executeAsync mapRowToTransaction
            |> Sql.tryExactlyOne
        member _.ListTransactionMatchCandidatesAsync(userId, cutoffDate) =
            connection
            |> Sql.query """
                SELECT * FROM foxybalance_transactions
                WHERE userid = @userId
                  AND matchedtransactionid IS NULL
                  AND status = 'Cleared'
                  AND datecreated >= @cutoffDate
                ORDER BY datecreated DESC, id DESC
                """
        ...
        member _.MatchTransactionsAsync(userId, id1, id2) =
            connection
            |> Sql.query """
                UPDATE foxybalance_transactions
                SET matchedtransactionid = CASE
                    WHEN id = @id1 THEN @id2
                    WHEN id = @id2 THEN @id1
                END
                WHERE userid = @userId AND id IN (@id1, @id2)
                """
            |> Sql.parameters [
                "userId", Sql.int userId
                "id1", Sql.int64 id1
        ...
        member _.UnmatchTransactionAsync(userId, id) =
            connection
            |> Sql.query """
                WITH matched AS (
                    SELECT matchedtransactionid FROM foxybalance_transactions
                    WHERE userid = @userId AND id = @id
                )
                UPDATE foxybalance_transactions
                SET matchedtransactionid = NULL
                WHERE userid = @userId AND (
                    id = @id OR id = (SELECT matchedtransactionid FROM matched)
                )
                """
            |> Sql.parameters [ "userId", Sql.int userId; "id", Sql.int64 id ]
            |> Sql.executeNonQueryAsync

        member _.GetBillMatchHistoryAsync(userId) =
            connection
            |> Sql.query """
                SELECT
                    UPPER(TRIM(name)) AS normalized_name,
                    recurringbillid AS bill_id,
                    COUNT(*) AS match_count
                FROM foxybalance_transactions
                WHERE userid = @userId
                  AND recurringbillid IS NOT NULL
                  AND autogenerated = false
                GROUP BY UPPER(TRIM(name)), recurringbillid
                """
            |> Sql.parameters [ "userId", Sql.int userId ]
            |> Sql.executeAsync (fun read ->
                read.string "normalized_name", read.int64 "bill_id", read.int "match_count")
            |> Sql.map (fun rows ->
                rows |> List.fold (fun map (name, billId, count) ->
                    Map.add (name, billId) count map) Map.empty)
