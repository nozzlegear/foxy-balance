...
type TransactionDatabaseTests(fixture: DbContainerFixture) =
        ...
        task {
            ...
            let partialTransaction: PartialTransaction =
                { Name = bogus.Commerce.Product()
                  ...
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
    ...
    [<Fact>]
    member _.``SumAsOfDateAsync should sum only transactions before the target date``() =
        task {
            // Setup
            let! user = createUser ()
            let baseDate = DateTimeOffset(2025, 6, 15, 12, 0, 0, TimeSpan.Zero)

            // Transaction before the cutoff (should be included)
            let tx1 =
                { Name = "Before Debit"
                  Amount = 100M
                  DateCreated = baseDate.AddDays(-10.0)
                  Status = Cleared (baseDate.AddDays(-9.0))
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! _ = database.CreateAsync(user.Id, tx1)

            ()

            // Transaction after the cutoff (should be excluded)
            let tx2 =
                { Name = "After Debit"
                  Amount = 500M
                  DateCreated = baseDate.AddDays(10.0)
                  Status = Cleared (baseDate.AddDays(11.0))
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! _ = database.CreateAsync(user.Id, tx2)

            ()

            // Act: sum as of baseDate (exclusive)
            let! result = database.SumAsOfDateAsync(user.Id, baseDate, true)

            // Assert: only tx1 (100M debit) is included
            %result.ClearedDebitSum.Should().Be(100M)
            %result.ClearedCreditSum.Should().Be(0M)
            %result.ClearedSum.Should().Be(-100M)
            %result.Sum.Should().Be(-100M)
        }

    [<Fact>]
    member _.``SumAsOfDateAsync with includePending=false should only sum cleared transactions``() =
        task {
            // Setup
            let! user = createUser ()
            let baseDate = DateTimeOffset(2025, 6, 15, 12, 0, 0, TimeSpan.Zero)

            // Cleared transaction before cutoff
            let tx1 =
                { Name = "Cleared Credit"
                  Amount = 200M
                  DateCreated = baseDate.AddDays(-5.0)
                  Status = Cleared (baseDate.AddDays(-4.0))
                  Type = TransactionType.Credit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! _ = database.CreateAsync(user.Id, tx1)
            ...
            // Pending transaction before cutoff
            let tx2 =
                { Name = "Pending Debit"
                  Amount = 300M
                  DateCreated = baseDate.AddDays(-3.0)
                  Status = Pending
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! _ = database.CreateAsync(user.Id, tx2)
            ...
            // Act: sum as of baseDate, excluding pending
            let! result = database.SumAsOfDateAsync(user.Id, baseDate, false)

            // Assert: only tx1 (200M cleared credit) is included
            %result.ClearedCreditSum.Should().Be(200M)
            %result.ClearedDebitSum.Should().Be(0M)
            %result.ClearedSum.Should().Be(200M)
            %result.Sum.Should().Be(200M)
        }

    [<Fact>]
    member _.``SumAsOfDateAsync should return zero sums when no transactions before date``() =
        task {
            // Setup
            let! user = createUser ()
            let futureDate = DateTimeOffset.UtcNow.AddDays(365.0)

            // Act
            let! result = database.SumAsOfDateAsync(user.Id, futureDate, true)
    ...
    [<Fact>]
    member _.``SumAsOfDateAsync should return zero sums for user with no transactions``() =
        task {
            // Setup
            let! user = createUser ()
            let anyDate = DateTimeOffset(2025, 1, 1, 0, 0, 0, TimeSpan.Zero)

            // Act
            let! result = database.SumAsOfDateAsync(user.Id, anyDate, true)
    ...
    [<Fact>]
    member _.``SumBeforeTransactionAsync should sum transactions before the target transaction's creation date``() =
        task {
            // Setup
            let! user = createUser ()
            let baseDate = DateTimeOffset(2025, 6, 15, 12, 0, 0, TimeSpan.Zero)

            // Earlier transaction (should be included in "before" sum)
            let tx1 =
                { Name = "Earlier Credit"
                  Amount = 150M
                  DateCreated = baseDate.AddDays(-5.0)
                  ...
                  Type = TransactionType.Credit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! created1 = database.CreateAsync(user.Id, tx1)
            ...
            let targetTx =
                { Name = "Target"
                  ...
                  Status = Cleared (baseDate.AddDays(1.0))
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! target = database.CreateAsync(user.Id, targetTx)

            // Later transaction (should be excluded from "before" sum)
            let tx3 =
                { Name = "Later Debit"
                  Amount = 500M
                  DateCreated = baseDate.AddDays(5.0)
                  Status = Cleared (baseDate.AddDays(6.0))
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! _ = database.CreateAsync(user.Id, tx3)

            ()

            // Act
            let! result = database.SumBeforeTransactionAsync(user.Id, target.Id, false)

            // Assert: only tx1 (150M credit) is before the target
            %result.ClearedCreditSum.Should().Be(150M)
            %result.ClearedDebitSum.Should().Be(0M)
            %result.ClearedSum.Should().Be(150M)
            %result.Sum.Should().Be(150M)
        }

    [<Fact>]
    member _.``SumBeforeTransactionAsync with useClearDate should use clear date for comparison``() =
        task {
            // Setup
            let! user = createUser ()
            let baseDate = DateTimeOffset(2025, 6, 15, 12, 0, 0, TimeSpan.Zero)

            // Transaction cleared before target's clear date (should be included)
            let tx1 =
                { Name = "Earlier Clear"
                  Amount = 100M
                  DateCreated = baseDate.AddDays(-10.0)
                  Status = Cleared (baseDate.AddDays(-3.0))
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! _ = database.CreateAsync(user.Id, tx1)
            ...
            // Target transaction with clear date at baseDate
            let targetTx =
                { Name = "Target"
                  Amount = 999M
                  DateCreated = baseDate.AddDays(-5.0)
                  Status = Cleared baseDate
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! target = database.CreateAsync(user.Id, targetTx)

            // Transaction cleared after target's clear date (should be excluded)
            let tx3 =
                { Name = "Later Clear"
                  Amount = 400M
            ...
            // Act
            let! result = database.SumBeforeTransactionAsync(user.Id, target.Id, true)

            // Assert: only tx1 (100M cleared debit) is cleared before target's clear date
            %result.ClearedDebitSum.Should().Be(100M)
            %result.ClearedCreditSum.Should().Be(0M)
            %result.ClearedSum.Should().Be(-100M)
            %result.Sum.Should().Be(-100M)
    ...
    [<Fact>]
    member _.``SumBeforeTransactionAsync should return zero sums when transaction does not exist``() =
        task {
            // Setup
            let! user = createUser ()
            let nonExistentId = 999999L
            ...
            // Act
            let! result = database.SumBeforeTransactionAsync(user.Id, nonExistentId, false)
    ...
    [<Fact>]
    member _.``SumAfterTransactionAsync should sum transactions after the target transaction's creation date``() =
        task {
            // Setup
            let! user = createUser ()
            let baseDate = DateTimeOffset(2025, 6, 15, 12, 0, 0, TimeSpan.Zero)

            // Earlier transaction (should be excluded from "after" sum)
            let tx1 =
                { Name = "Earlier Credit"
                  Amount = 150M
                  DateCreated = baseDate.AddDays(-5.0)
            ...
            // Later transaction (should be included in "after" sum)
            let tx3 =
                { Name = "Later Debit"
                  Amount = 500M
                  DateCreated = baseDate.AddDays(5.0)
            ...
            // Act
            let! result = database.SumAfterTransactionAsync(user.Id, target.Id, false)

            // Assert: only tx3 (500M debit) is after the target
            %result.ClearedDebitSum.Should().Be(500M)
            %result.ClearedCreditSum.Should().Be(0M)
            %result.ClearedSum.Should().Be(-500M)
            %result.Sum.Should().Be(-500M)
        }

    [<Fact>]
    member _.``SumAfterTransactionAsync with useClearDate should use clear date for comparison``() =
        task {
            // Setup
            let! user = createUser ()
            let baseDate = DateTimeOffset(2025, 6, 15, 12, 0, 0, TimeSpan.Zero)

            // Transaction cleared before target's clear date (should be excluded)
            let tx1 =
                { Name = "Earlier Clear"
                  Amount = 100M
                  DateCreated = baseDate.AddDays(-10.0)
            ...
            // Transaction cleared after target's clear date (should be included)
            let tx3 =
                { Name = "Later Clear"
                  Amount = 400M
                  DateCreated = baseDate.AddDays(-1.0)
            ...
            // Act
            let! result = database.SumAfterTransactionAsync(user.Id, target.Id, true)

            // Assert: only tx3 (400M cleared credit) is cleared after target's clear date
            %result.ClearedCreditSum.Should().Be(400M)
            %result.ClearedDebitSum.Should().Be(0M)
            %result.ClearedSum.Should().Be(400M)
            %result.Sum.Should().Be(400M)
        }

    [<Fact>]
    member _.``SumAfterTransactionAsync should return zero sums when transaction does not exist``() =
        task {
            // Setup
            let! user = createUser ()
            let nonExistentId = 999999L
            ...
            // Act
            let! result = database.SumAfterTransactionAsync(user.Id, nonExistentId, false)
        ...
        task {
            ...
            let transactions = [
                { Name = "Imported Transaction 1"
                  ...
                  Status = Pending
                  Type = Debit
                  ImportId = Some "import-123"; RecurringBillId = None; AutoGenerated = false }
                { Name = "Imported Transaction 2"
                  Amount = 200M
                  DateCreated = baseDate.AddDays(1.0)
                  Status = Cleared (bogus.Date.RecentOffset 5)
                  Type = Credit
                  ImportId = Some "import-456"; RecurringBillId = None; AutoGenerated = false }
            ]
        ...
        task {
            ...
            let transactions = [
                { Name = "With ImportId"
                  ...
                  Status = Pending
                  Type = Debit
                  ImportId = Some "import-789"; RecurringBillId = None; AutoGenerated = false }
        ...
        task {
            ...
            let partialTransaction: PartialTransaction =
                { Name = "Transaction with ImportId"
                  ...
                  Status = Pending
                  Type = Debit
                  ImportId = Some "create-import-123"; RecurringBillId = None; AutoGenerated = false }
        ...
        task {
            ...
            let originalTransaction: PartialTransaction =
                { Name = "Original Transaction"
                  ...
                  Status = Pending
                  Type = Debit
                  ImportId = Some "update-import-123"; RecurringBillId = None; AutoGenerated = false }
        ...
        task {
            ...
            let initialTransactions = [
                { Name = "Existing Transaction 1"
                  ...
                  Status = Pending
                  Type = Debit
                  ImportId = Some "import-duplicate-1"; RecurringBillId = None; AutoGenerated = false }
                { Name = "Existing Transaction 2"
                  Amount = 200M
                  DateCreated = bogus.Date.RecentOffset 19
                  Status = Cleared (bogus.Date.RecentOffset 5)
                  Type = Credit
                  ImportId = Some "import-duplicate-2"; RecurringBillId = None; AutoGenerated = false }
            ]
            ...
            let duplicateTransactions = [
                ...
                { Name = "New Transaction"  // Should be imported
                  ...
                  Status = Pending
                  Type = Credit
                  ImportId = Some "import-new-1"; RecurringBillId = None; AutoGenerated = false }
                ...
                { Name = "Another New Transaction"  // Should be imported
                  ...
                  Status = Pending
                  Type = Credit
                  ImportId = Some "import-new-2"; RecurringBillId = None; AutoGenerated = false }
            ]
        ...
        task {
            ...
            let transaction1 = [
                { Name = "User 1 Transaction"
                  ...
                  Status = Pending
                  Type = Debit
                  ImportId = Some "shared-import-id"; RecurringBillId = None; AutoGenerated = false }
            ]
    ...
    [<Fact>]
    member _.``CreateAsync should handle RecurringBillId when provided``() =
        task {
            // Setup
            let! user = createUser ()
            ...
            // Assert
            %result.RecurringBillId.Should().Be(Some bill.Id)
            %result.AutoGenerated.Should().BeTrue()

            // Verify it was persisted
            let! retrieved = database.GetAsync(user.Id, result.Id)
            %retrieved.Should().BeSome()
            %retrieved.Value.RecurringBillId.Should().Be(Some bill.Id)
            %retrieved.Value.AutoGenerated.Should().BeTrue()
        }

    [<Fact>]
    member _.``CreateAsync should handle missing RecurringBillId``() =
        task {
            // Setup
            let! user = createUser ()
            let partialTransaction: PartialTransaction =
            ...
            // Assert
            %result.RecurringBillId.Should().BeNone()
            %result.AutoGenerated.Should().BeFalse()

            // Verify it was persisted
            let! retrieved = database.GetAsync(user.Id, result.Id)
            %retrieved.Should().BeSome()
            %retrieved.Value.RecurringBillId.Should().BeNone()
            %retrieved.Value.AutoGenerated.Should().BeFalse()
        }

    [<Fact>]
    member _.``UpdateAsync should update RecurringBillId``() =
        task {
            // Setup
            let! user = createUser ()
    ...
    [<Fact>]
    member _.``BulkCreateAsync should handle RecurringBillId and AutoGenerated``() =
        task {
            // Setup
            let! user = createUser ()
            ...
            let transactions = [
                ...
                { Name = "Manual transaction"
                  ...
                  RecurringBillId = None
                  AutoGenerated = false }
                { Name = "Imported and matched"
                  Amount = 150M
                  DateCreated = bogus.Date.RecentOffset 18
                  Status = Pending
                  Type = Debit
                  ImportId = Some "import-match-123"
                  RecurringBillId = Some bill2.Id
                  AutoGenerated = false }
            ]
            ...
            %transactionsList.Should().HaveLength(3)

            let autoGen = transactionsList |> List.find (fun t -> t.Name = "Auto-generated 1")
            %autoGen.RecurringBillId.Should().Be(Some bill1.Id)
            %autoGen.AutoGenerated.Should().BeTrue()

            let manual = transactionsList |> List.find (fun t -> t.Name = "Manual transaction")
            %manual.RecurringBillId.Should().BeNone()
            %manual.AutoGenerated.Should().BeFalse()

            let matched = transactionsList |> List.find (fun t -> t.Name = "Imported and matched")
            %matched.RecurringBillId.Should().Be(Some bill2.Id)
            %matched.AutoGenerated.Should().BeFalse()
            %matched.ImportId.Should().Be(Some "import-match-123")
        }

    [<Fact>]
    member _.``ListAsync should return transactions with RecurringBillId``() =
        task {
            // Setup
            let! user = createUser ()
            ...
            let! _ = database.CreateAsync(user.Id, linkedTransaction)

            // Create unlinked transaction
            let unlinkedTransaction: PartialTransaction =
                { Name = "Not linked"
                  Amount = 50M
                  DateCreated = bogus.Date.RecentOffset 19
                  Status = Pending
            ...
            let! _ = database.CreateAsync(user.Id, unlinkedTransaction)
            ...
            let linked = allTransactions |> Seq.find (fun t -> t.Name = "Linked to bill")
            let unlinked = allTransactions |> Seq.find (fun t -> t.Name = "Not linked")

            %linked.RecurringBillId.Should().Be(Some bill.Id)
            %linked.AutoGenerated.Should().BeTrue()
            %unlinked.RecurringBillId.Should().BeNone()
            %unlinked.AutoGenerated.Should().BeFalse()
        }

    [<Fact>]
    member _.``MatchTransactionsAsync should link two transactions reciprocally``() =
        task {
            // Setup
            let! user = createUser ()
            let clearedDate = bogus.Date.RecentOffset 5
            let tx1: PartialTransaction =
                { Name = "Transaction A"
                  Amount = 100M
                  DateCreated = bogus.Date.RecentOffset 10
                  Status = Cleared clearedDate
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let tx2: PartialTransaction =
                { Name = "Transaction B"
                  Amount = 50M
                  DateCreated = bogus.Date.RecentOffset 8
                  Status = Cleared clearedDate
                  Type = TransactionType.Credit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }

            let! created1 = database.CreateAsync(user.Id, tx1)
            let! created2 = database.CreateAsync(user.Id, tx2)

            // Verify initial state: no match
            let! result1Before = database.GetAsync(user.Id, created1.Id)
            %result1Before.Value.MatchedTransactionId.Should().BeNone()

            // Act
            let! rowsUpdated = database.MatchTransactionsAsync(user.Id, created1.Id, created2.Id)
            %rowsUpdated.Should().Be(2)

            // Assert: both transactions should reference each other
            let! result1 = database.GetAsync(user.Id, created1.Id)
            %result1.Value.MatchedTransactionId.Should().Be(Some created2.Id)

            let! result2 = database.GetAsync(user.Id, created2.Id)
            %result2.Value.MatchedTransactionId.Should().Be(Some created1.Id)
        }

    [<Fact>]
    member _.``MatchTransactionsAsync should exclude matched transactions from ListTransactionMatchCandidatesAsync``() =
        task {
            // Setup
            let! user = createUser ()
            let clearedDate = bogus.Date.RecentOffset 5
            ...
            let tx3: PartialTransaction =
                { Name = "Unmatched"
                  Amount = 25M
                  DateCreated = bogus.Date.RecentOffset 6
                  Status = Cleared clearedDate
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! created3 = database.CreateAsync(user.Id, tx3)

            // Act
            let cutoffDate = System.DateTimeOffset.UtcNow.AddDays(-45.0)
            let! candidates = database.ListTransactionMatchCandidatesAsync(user.Id, cutoffDate)

            // Assert: only unmatched transaction should appear
            let candidateList = List.ofSeq candidates
            %candidateList.Should().HaveLength(1)
            %candidateList.[0].Name.Should().Be("Unmatched")
        }

    [<Fact>]
    member _.``UnmatchTransactionAsync should clear both sides of a match``() =
        task {
            // Setup
            let! user = createUser ()
            let clearedDate = bogus.Date.RecentOffset 5
            ...
            // Act
            let! rowsUpdated = database.UnmatchTransactionAsync(user.Id, created1.Id)
            %rowsUpdated.Should().Be(2)

            // Assert: both should be cleared
            let! result1After = database.GetAsync(user.Id, created1.Id)
            %result1After.Value.MatchedTransactionId.Should().BeNone()

            let! result2After = database.GetAsync(user.Id, created2.Id)
            %result2After.Value.MatchedTransactionId.Should().BeNone()
        }

    [<Fact>]
    member _.``UnmatchTransactionAsync is a no-op on unmatched transaction``() =
        task {
            // Setup
            let! user = createUser ()
            let clearedDate = bogus.Date.RecentOffset 5
            let tx: PartialTransaction =
                { Name = "Already Unmatched"
                  Amount = 75M
                  DateCreated = bogus.Date.RecentOffset 10
                  Status = Cleared clearedDate
                  Type = TransactionType.Debit
            ...
            let! created = database.CreateAsync(user.Id, tx)

            // Act: unmatch an already-unmatched transaction
            let! rowsUpdated = database.UnmatchTransactionAsync(user.Id, created.Id)
            %rowsUpdated.Should().Be(1)  // Updates the source row (idempotent)

            // Assert: should still be unmatched (no error)
            let! result = database.GetAsync(user.Id, created.Id)
            %result.Value.MatchedTransactionId.Should().BeNone()
        }

    [<Fact>]
    member _.``MatchTransactionsAsync should only match transactions belonging to the user``() =
        task {
            // Setup
            let! user1 = createUser ()
            let! user2 = createUser ()
            ...
            let tx1: PartialTransaction =
                { Name = "User1 Transaction"
                  ...
                  Type = TransactionType.Debit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! created1 = database.CreateAsync(user1.Id, tx1)
            ...
            let tx2: PartialTransaction =
                { Name = "User2 Transaction"
                  ...
                  Type = TransactionType.Credit
                  ImportId = None; RecurringBillId = None; AutoGenerated = false }
            let! created2 = database.CreateAsync(user2.Id, tx2)

            // Act: user1 tries to match their transaction with user2's
            let! rowsUpdated = database.MatchTransactionsAsync(user1.Id, created1.Id, created2.Id)

            // Assert: only user1's transaction should be updated (row count = 1)
            %rowsUpdated.Should().Be(1)

            // user1's transaction should reference user2's
            let! result1 = database.GetAsync(user1.Id, created1.Id)
            %result1.Value.MatchedTransactionId.Should().Be(Some created2.Id)

            // user2's transaction should NOT be affected (user2 can't see the match)
            let! result2 = database.GetAsync(user2.Id, created2.Id)
            %result2.Value.MatchedTransactionId.Should().BeNone()
        }
