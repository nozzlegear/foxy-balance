...
deploy-secrets sshTarget secretFile:
    ...
    try {
        ...
        ssh {{ssh_opts}} $sshTarget 'PG_USER=$(jq -r .Postgres_Username /tmp/appsettings.secrets.json); podman secret rm foxybalance_pg_username 2>/dev/null || true; printf "%s" "$PG_USER" | podman secret create foxybalance_pg_username -'
        ssh {{ssh_opts}} $sshTarget 'PG_PASS=$(jq -r .Postgres_Password /tmp/appsettings.secrets.json); podman secret rm foxybalance_pg_password 2>/dev/null || true; printf "%s" "$PG_PASS" | podman secret create foxybalance_pg_password -'
